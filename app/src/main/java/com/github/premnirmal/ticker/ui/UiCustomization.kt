...
/**
 * Font helpers for the "白い熊 株価表示 UI" customization page (Phase 2, global scope).
 *
...
/**
 * Rebuilds a [Typography] from the user's per-category (heading/body) font choices. Display/headline/
 * title styles use the heading font/weight/colour/scale; body/label styles use the body ones. [base]
 * is the shared brand typography (from `appTypography()`), passed in because it is `@Composable`.
 * Font families come from [FontManager]; colours use the [COLOUR_UNSET] sentinel via [toOverrideColour].
 */
...
fun buildAppTypography(
    ...
    val hColour = headingColour.toOverrideColour()
    val bColour = bodyColour.toOverrideColour()
